...
class VideoProcessor {
    ...
    init(processDimension: ProcessDimension) throws {
        ...
        guard let sampler = metalDevice.makeSamplerState(descriptor: samplerDescriptor) else {
            throw VideoProcessorError.metalSetupFailed("Failed to create texture sampler")
        }
        ...
        guard result == kCVReturnSuccess, let cache = textureCache else {
            throw VideoProcessorError.metalSetupFailed("Failed to create texture cache")
        }
        self.textureCache = cache
        
        // Load and compile shaders
        guard let defaultLibrary = metalDevice.makeDefaultLibrary() else {
            throw VideoProcessorError.metalSetupFailed("Failed to create default library")
        }
        ...
        do {
            ...
            throw VideoProcessorError.metalSetupFailed("Failed to create superview compute pipeline state: \(error)")
        }
    ...
    func convertVideo(inputURL: URL, outputURL: URL, progressCallback: @escaping (Float) -> Void) async throws {
        ...
        guard writer.startWriting() else {
            throw VideoProcessorError.writerFailed(writer.error?.localizedDescription ?? "Unknown error")
        }
    ...
    private func processFrameWithShader(
        ...
        guard let outputYTexture = createYTexture(from: outputBuffer),
            ...
            throw VideoProcessorError.textureCreationFailed("Failed to create output YUV textures for \(shaderName)")
        }
        
        // Execute shader
        guard let commandBuffer = commandQueue.makeCommandBuffer(),
              let encoder = commandBuffer.makeComputeCommandEncoder() else {
            throw VideoProcessorError.metalExecutionFailed("Failed to create command buffer or encoder for \(shaderName)")
        }
        ...
        if let error = commandBuffer.error {
            throw VideoProcessorError.metalExecutionFailed("\(shaderName): \(error.localizedDescription)")
        }
